...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['DATALEON_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DATALEON_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Dataleon API.
 */
export class Dataleon {
   ...
   * API Client for interfacing with the Dataleon API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['DATALEON_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['DATALEON_BASE_URL'] ?? https://inference.eu-west-1.dataleon.ai] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DATALEON_BASE_URL'),
    apiKey = readEnv('DATALEON_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.DataleonError(
        "The DATALEON_API_KEY environment variable is missing or empty; either provide it, or instantiate the Dataleon client with an apiKey option, like new Dataleon({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://inference.eu-west-1.dataleon.ai`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Dataleon.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DATALEON_LOG'), "process.env['DATALEON_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('DATALEON_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://inference.eu-west-1.dataleon.ai';
  }
  ...
  static Dataleon = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static DataleonError = Errors.DataleonError;
