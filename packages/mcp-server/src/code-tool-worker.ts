...
import { Dataleon, ClientOptions } from '@dataleon/dataleon';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Dataleon)`
    : `const run: (${functionSource.client}: Dataleon) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.individuals.update',
    'client.individuals.documents.list',
    ...
    'client.companies.documents.list',
    'client.companies.documents.upload',
  ],
